...
int main() {
	while (true) {
		...
		vector<int> datavalues(size, 5);
		vector<int> datakey{0};
		for (int n = 0; n < size; n++)
		{
			datakey.push_back(n+1);
		}
		...
		int value = 1;
		binarySearch(datakey, datavalues, size+2, value);
		...
		sequentialSearch(datakey, datavalues, size+2, value);
